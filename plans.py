...
class BaxterInterfacePlanner(object):
    ...
    def ik_joint_and_gripper_plan_execution(self, waypoint_list, gripper_last=True):
    ...
    def guided_move_to_joint(self, pose, gripper_pos=None):
        ...
        ------
                self._limb.move_to_joint_positions(goal_joints, timeout=5.0)
                se_action = [Pose1, Pose2, Pose3, ..., PoseN]
    ...
    def move_to_joint_position(self, joints):
        """check the djoiny dimensions if has to be dict or list
        """
        self._limb.move_to_joint_positions(joints, timeout=5.0)
        return 0
